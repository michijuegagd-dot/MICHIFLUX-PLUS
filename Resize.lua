...
local lp = Plrs.LocalPlayer local pGui = lp:WaitForChild("PlayerGui") local cg = game:GetService("CoreGui")
...
local minS, maxS, isDrag = 0.05, 25.0, false local originalSizes = {}
...
local function resize(scale)
    ...
    local h = c:FindFirstChildOfClass("Humanoid") if not h then return end local isR15 = (h.RigType == Enum.RigType.R15)
    ...
    if rp then Stabilizer.Parent = rp Stabilizer.MaxTorque = (scale <= 0.15) and Vector3.new(400000, 0, 400000) or Vector3.new(0, 0, 0) end
    if isR15 then
        ...
        end pcall(function() c:ScaleTo(scale) end)
    else
        for _, part in ipairs(c:GetChildren()) do if part:IsA("BasePart") and originalSizes[part.Name] then part.Size = originalSizes[part.Name] * scale end end
    end
    ...
    for _, part in ipairs(c:GetChildren()) do
        if part:IsA("BasePart") then part.Massless = true
    ...
    lp.CameraMaxZoomDistance = math.clamp(400 * scale, 128, 5000) lp.CameraMinZoomDistance = math.clamp(0.5 * scale, 0.1, 10)
...
RS.RenderStepped:Connect(function()
    if isDrag then
        ...
        SliderBtn.Position = UDim2.new(p, -8, 0, -5) local s = math.round((minS + (p * (maxS - minS))) * 100) / 100
